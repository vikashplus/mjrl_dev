...
def gather_training_data(env, data, filename='demo_playback.mp4', render=None):
    ...
    return path_obs, path_act, path_reward, path_env_infos, init_qpos, init_qvel
...
def main(env, demo_dir, skip, graph, save_logs, view, render):
    ...
    for ind, file in enumerate(glob.glob(demo_dir + "*.mjl")):
        ...
        # playback logs and gather data
            ...
            try:
                obs, act, rew, env_infos, init_qpos, init_qvel = gather_training_data(gym_env, data,\
    ...
    pickle.dump(paths, open(demo_dir + env + "_all_paths.pkl", 'wb'))
    print("Saved all trajectories")
